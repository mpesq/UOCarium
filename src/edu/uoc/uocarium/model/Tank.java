...
public class Tank implements Comparable<Tank>{
	 ...
	 */	
	public void addItem(Item item) throws TankException, ItemException{
		if(item == null) {
		...
		items.add(item);
		item.setTank(this);
		UOCariumController Controller = new UOCariumController("./files/");
		for(Keeper keeper : Controller.getKeepers())	{
			for(Tank tank : keeper.getTanks())	{
				if(tank.getId().equals(this.id)) item.setIdKeeper(keeper.getId());
			}
		}
	...
	public String toString() {
		...
		str.append("\nID: " + getId());	//He posat per a que es vegi id del tank
		str.append("\n"+getDescription());
		str.append("\nLength: "+getLength());
		str.append("\nHeight: "+getHeight());
	...
	/**
	 * Override de compareTo perque ordeni alfabeticament dos Tanks segons el seu id
	 */
